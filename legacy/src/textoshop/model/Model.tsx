...
function inferBaseUrl() {
    ...
    const isDev = typeof window !== 'undefined' && typeof location !== 'undefined' && location.host.includes(':');
    return isDev ? `${location.origin}/openai` : 'https://api.openai.com/v1';
}
...
function getInitialState() {
    ...
    const startingLayers = [
        { id: "1", layer: { name: "Layer 1", color: "white", isVisible: false, state: [{
            //@ts-ignore
            type: "paragraph",
    ...
    const initialState: ModelState = {
        ...
        tone: [
            { lowAdjective: "Casual and Modern", highAdjective: "Formal and Archaic", value: 5 },
            { lowAdjective: "Gentle and Soft", highAdjective: "Forceful and Commanding", value: 5 },
            { lowAdjective: "Calm and Restrained", highAdjective: "Excited and Intense", value: 5 },
        ],
        ...
        if (executablePrompt.response_format) {
            ...
            try {
                stream = await openai.chat.completions.create({
                    model: executablePrompt.model ? executablePrompt.model : 'gpt-4o-mini',
                    messages: [{ role: 'user', content: executablePrompt.prompt }],
                    stream: true,
                    temperature: 0,
