...
import { MoveableTextField, textFieldEditors, tools, useReviewModelStore } from "../../../model/review/ReviewModel";
import { SelectionTool } from "../../../model/review/tools/toolbarTools/SelectionTool";
import { ToolTextSelectionEvent } from "../../../model/review/ToolbarTool";
import { TextLayerUtils } from "../../../model/utils/TextLayerUtils";
import { DraggingParameters, useReviewViewModelStore } from "../../../model/review/ViewModel";
...
const Leaf = (props: any) => {
    ...
    if (props.leaf.layerTagId) {
        ...
        TextLayerUtils.flattenHierarchicalLayers(useReviewModelStore.getState().layers).forEach((hlayer) => {
    ...
    if (props.leaf.removed) {
        ...
        const tone = useReviewModelStore.getState().tone;
        (style as any)['--tone-color'] = `${tone[0].value/10*255}, ${tone[1].value/10*255}, ${tone[2].value/10*255}`;
    }

    // Non-intrusive review overlays drawn via background gradient using global window.reviewOverlays
    try {
        const overlays = (window as any).reviewOverlays && (window as any).reviewOverlays[props.editor?.id || 'translationField']
        if (overlays && Array.isArray(overlays)) {
            const text = props.children?.props?.text?.text as string || ''
            if (text && text.length > 0) {
                // For simplicity, if any overlay exists, use subtle underline, tooltip via title
                // Precise per-span mapping is complex; keep non-blocking styling
                (style as any).backgroundImage = (style as any).backgroundImage || 'linear-gradient(transparent, transparent)'
                if (!props.attributes?.title) (props.attributes as any).title = props.leaf.selectionTooltip
            }
        }
    } catch {}
...
export default function EditableTextField(props: {style?: React.CSSProperties, isMoveable?: boolean, textField: MoveableTextField, className?: string}) {
    const selectedToolStr = useReviewModelStore(state => state.selectedTool);
    const selectedTool = tools[selectedToolStr];
    const [isLoading, setIsLoading] = useState(false);
    const startDragging = useReviewViewModelStore(state => state.startDragging);
    const dragging = useReviewViewModelStore(state => state.dragging);
    const inMultipleSelectionMode = useReviewModelStore(state => state.inMultipleSelectionMode);
    const [isResizing, setIsResizing] = useState(false);
    const textFieldDivRef = React.createRef<HTMLDivElement>();

    const selectedTexts = useReviewModelStore(state => state.selectedTexts);
    const setSelectedTexts = useReviewModelStore(state => state.setSelectedTexts);

    const setTextFields = useReviewModelStore(state => state.setTextFields);

    const isDragging = useReviewViewModelStore(state => state.dragging);
    ...
    const onTextSelected = (selectionRange: Range) => {
        ...
        if (slateSelectionRange) {
            ...
            useReviewModelStore.getState().getSelectedTool().onTextSelected(event);
            ...
            if (!event.preventTextSelection) {
                ...
                editor.addMark("selectionClassname", useReviewModelStore.getState().getSelectedTool().getMarksClassname());
                editor.deselect();

                // If this is a selection, we should also reset the tones
                if (useReviewModelStore.getState().selectedTool === SelectionTool.getToolName()) {
                    useReviewModelStore.getState().setTone(
                        useReviewModelStore.getState().tone.map(e => ({...e, value: 5}))
                    )
                }
            }
        }
    ...
    useEffect(() => {
        ...
        const onMouseUp = (e: MouseEvent) => { 
            if (e.button === 0 && isResizing) {
                ...
                const rect = textFieldDivRef.current?.getBoundingClientRect();
                if (rect) {
                    useReviewModelStore.getState().setTextField(props.textField.id, {x: rect.left, y: rect.top, width: rect.width, height: rect.height});
                }
            }
        }
        ...
        style={{ position: 'absolute', boxShadow: 'rgba(0, 0, 0, 0.16) 0px 10px 36px 0px, rgba(0, 0, 0, 0.06) 0px 0px 0px 1px', borderRadius: 4, /*padding: 10,*/ left: textField.x, top: textField.y, minHeight: textField.height, width: textField.width, background: 'white' , /*cursor: props.isMoveable ? 'pointer' : undefined,*/ ...props.style}}>
            ...
            <TextSelection disabled={isDragging} opaque={selectedTool.isSelectionOpaque()} className={selectedTool.getMarksClassname()} style={style} onTextSelected={onTextSelected}>
                ...
                <Slate data-gramm={false} data-gramm_editor={false} data-enable-grammarly={false} editor={editor} initialValue={textField.state}  onChange={newValue => {
                    ...
                    setTextFields(useReviewModelStore.getState().textFields.map(e => e.id === props.textField.id ? {...e, state: newValue} : e));
