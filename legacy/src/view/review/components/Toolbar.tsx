...
import { tools, useReviewModelStore } from "../../../model/review/ReviewModel";
import { ToolbarTool } from "../../../model/review/ToolbarTool";
...
export function ToolbarButton(props: {tool: ToolbarTool, shortcutButton?: string, toolVariants: ToolbarTool[]}) {
  const setSelectedTool = useReviewModelStore(state => state.setSelectedTool);
    ...
    <DropdownMenu variant="flat">
      {props.toolVariants.map((tool) => {
        return <DropdownItem textValue={tool.name} key={tool.name} onClick={() => {
          ...
          const toolOrder = useReviewModelStore.getState().toolsOrderInToolbar;
          ...
          useReviewModelStore.getState().setToolOrderInToolbar(newToolOrder);
          useReviewModelStore.getState().setSelectedTool(tool.name);
        }}>
...
export function Toolbar() {
  const setSelectedTool = useReviewModelStore(state => state.setSelectedTool);
  const undoStack = useReviewUndoModelStore(state => state.undoStack);
  const redoStack = useReviewUndoModelStore(state => state.redoStack);
  const toolsOrderInToolbar = useReviewModelStore(state => state.toolsOrderInToolbar);
  ...
  return (
    <div id="leftToolbar" className='drop-shadow-md' style={{ position: 'fixed', left: 20, top: 20, padding: 5, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingBottom: 15, paddingTop: 15, background: 'white', borderRadius: 14, zIndex: 1000 }}>
          ...
          <Tooltip showArrow delay={0} closeDelay={0} content={"Undo"} placement="right" offset={0}>
            <Button isDisabled={undoStack.length === 0} style={{color: '#71717a', fontSize: 16}} isIconOnly size={"sm"} variant={"light"} onClick={() => useReviewUndoModelStore.getState().undo()}><IoIosUndo/></Button>
          </Tooltip>
          <Tooltip showArrow delay={0} closeDelay={0} content={"Redo"} placement="right" offset={0}>
            <Button isDisabled={redoStack.length === 0} style={{color: '#71717a', fontSize: 16}} isIconOnly size={"sm"} variant={"light"} onClick={() => useReviewUndoModelStore.getState().redo()}><IoIosUndo style={{transform: 'scaleX(-1)'}}/></Button>
          </Tooltip>
        </div>
  )
}
