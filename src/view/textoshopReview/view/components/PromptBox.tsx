...
export function PromptBox() {
  ...
  useEffect(() => {
    ...
    if (selectedTextSpans.length > 0) {
      // Find the rectangle that is the lowest (i.e. highest y value), use viewport coordinates
      const firstRect = selectedTextSpans[0].getBoundingClientRect();
      let newPosition = { x: firstRect.x + firstRect.width/2, y: firstRect.y + firstRect.height + 10 };
      for (let i = 1; i < selectedTextSpans.length; i++) {
        const rect = selectedTextSpans[i].getBoundingClientRect();
        if (rect.y + rect.height > newPosition.y) {
          newPosition = { x: rect.x + rect.width/2, y: rect.y + rect.height + 10 };
        }
      }
      // No need to subtract bgRect since we're using fixed positioning with viewport coordinates
