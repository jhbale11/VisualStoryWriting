...
import { HiearchicalLayer, useReviewModelStore } from "../../../model/review/ReviewModel";
...
export function LayerListElement(props: NodeRendererProps<HiearchicalLayer>) {
  const animateNextChanges = useReviewModelStore(state => state.animateNextChanges);
  const setLayerVisibility = useReviewModelStore(state => state.setLayerVisibility);
  const selectedLayerId = useReviewModelStore(state => state.selectedLayerId);
  ...
  onMouseLeave={() => {
    ...
    <button className={props.node.data.layer.isVisible ? "group-[:not(:hover)]:opacity-0" : "opacity-1"} style={{ marginRight: 2, position: 'absolute', right: 5 }}
      onClick={e => {
        useReviewUndoModelStore.getState().setIsDisabled(true);
        ...
        e.stopPropagation();
        useReviewUndoModelStore.getState().setIsDisabled(false);
      }}
    >
      {isVisible && <RiEyeFill />}
...
export function LayerManager() {
  const layers = useReviewModelStore(state => state.layers);
  const flattenedLayers = TextLayerUtils.flattenHierarchicalLayers(layers);
  const setSelectedLayer = useReviewModelStore(state => state.setSelectedLayer);
  const selectedLayerId = useReviewModelStore(state => state.selectedLayerId);
    ...
    <CardBody style={{ margin: 0, padding: 0 }}>
      <div ref={ref} style={{ display: 'flex', flexDirection: 'column', maxHeight: '100%' }}>
        <Tree data={[...layers].reverse()}
          ...
          onRename={({id, name}) => {
            useReviewUndoModelStore.getState().setIsDisabled(true);
            useReviewModelStore.getState().setLayerProperties(parseInt(id), {name: name});
            useReviewUndoModelStore.getState().setIsDisabled(false);
          }}
          ...
          onDelete={({ ids }) => {
            /*if (ids.length > 0) {
              useReviewModelStore.getState().removeLayer(parseInt(ids[0]));
            }*/
          }}
          ...
          onMove={({ dragIds, parentId, index }) => {
            if (dragIds.length > 0) {
              ...
              useReviewModelStore.getState().moveLayer(parseInt(dragIds[0]), parentId ? parseInt(parentId) : null, reversedIndex);
            }
          }}
        ...
        <div style={{ display: 'flex', justifyContent: 'right', margin: 10, gap: 5 }}>
          ...
          <Tooltip placement="bottom" content="Add new layer" closeDelay={0}>
            <Button size="sm" onFocus={(e) => (e.target as any)!.blur()} isIconOnly onClick={() => {
              ...
              const selectedLayer = useReviewModelStore.getState().selectedLayerId;
              ...
              const selectedTexts = useReviewModelStore.getState().selectedTexts.filter(e => e.textFieldId === "mainTextField");
              const modifications : {[tag: string]: string} = {};
              if (selectedTexts.length > 0 && useReviewModelStore.getState().selectedLayerId === 0) {
                // We should use the selected text to populate the new layer

                // First, we calculate the new layer 0
                const recoveredFirstLayer = TextLayerUtils.recoverFirstLayerFromState(useReviewModelStore.getState().textFields[0].state, TextLayerUtils.flattenHierarchicalLayersToTextLayer(layers));
                ...
                useReviewModelStore.getState().layers[0].layer.state = state;
              }

              const newLayer: HiearchicalLayer = { id: '', layer: { name: 'New layer', color: color, isVisible: true, modifications: modifications }, children: [] };
              useReviewModelStore.getState().addLayer(newLayer, parentLayerId, selectedLayerInfo.indexWithinParent + 1);
            }
          ...
          <Tooltip placement="bottom" content="Delete selected layer" closeDelay={0}>
            <Button isDisabled={selectedLayerId === 0} onFocus={(e) => (e.target as any)!.blur()} size="sm" isIconOnly onClick={() => {
              const selectedLayer = useReviewModelStore.getState().selectedLayerId;
              useReviewUndoModelStore.getState().storeUndoState();
              useReviewModelStore.getState().removeLayer(selectedLayer);
            }}><FaTrashAlt /></Button>
          </Tooltip>
        </div>
      </div>
